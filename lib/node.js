...
var WS_SECRET = process.env.WS_SECRET || "net-stats-has-a-secret";
...
Node.prototype.getPoolStatus = function()
{
	var self = this;
	var now = _.now();
	...
	if (this._web3)
	{
		 console.stats('==>', 'Getting Pool Status')

		web3.txpool.status(function (err, status)
		{
			if (err) {
				console.error('xx>', 'pool status error: ', err);
				return false;
			}
			...
			console.sstats('==>', 'Got', chalk.reset.red(status.pending) , chalk.reset.bold.green('pending tx'+ (pending === 1 ? '' : 's') + ' in'), chalk.reset.cyan(results.diff, 'ms'));
			console.sstats('==>', 'Got', chalk.reset.red(status.queued) , chalk.reset.bold.green('queued tx'+ (queued === 1 ? '' : 's') + ' in'), chalk.reset.cyan(results.diff, 'ms'));

			self.stats.pending = status.pending;
			self.stats.queued = status.queued;

			if(self._lastPending !== status.pending)
				self.sendPendingUpdate();
            self._lastPending = status.pending;
            if(self._lastQueued !== status.queued) {
                self.sendQueuedUpdate();
			self._lastQueued = status.queued
            }
        });
	}
}
...
Node.prototype.prepareQueued = function ()
{
	return {
		id: this.id,
		stats: {
...
Node.prototype.sendQueuedUpdate = function()
{
	console.stats("wsc", "Sending queued update");
	this.emit('queued', this.prepareQueued());
}
...
Node.prototype.setFilters = function()
{
	...
	try {
		this.queuedFilter = web3.eth.filter('queued');
		this.queuedFilter.watch( function (err, hash)
		{
			var now = _.now();
			var time = now - self._lastQueuedLog;
			self._lastQueuedLog = now;

			 console.stats('>>>', 'Queued Filter triggered:', chalk.reset.red(hash), '- last trigger:', chalk.reset.cyan(time));
			...
			{
				self._debouncedQueued();
			}
		});

		console.success("Installed queued filter");
	}
	catch (err)
	{
		this.queuedFilter = false;

		console.error("Couldn't set up queued filter");
		console.error(err);
	}
}
